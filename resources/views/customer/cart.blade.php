    ...
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center mb-4 gap-2">
        <div>
            ...
            <p class="text-muted small mb-0 d-none d-md-block">
                Cek kembali pesanan Anda sebelum checkout
            </p>
        </div>
        <div class="d-flex gap-2">
            ...
            <a href="{{ route('customer.tenants') }}" class="btn btn-sm btn-primary">
                ...
                <span class="d-none d-sm-inline">Belanja</span>
            </a>
        </div>
    </div>
                ...
                <div class="card-body p-2 p-md-3">
                    ...
                    <div class="cart-items">
                        @foreach($cart->items as $item)
                            ...
                            @endphp
                            <div class="cart-item d-flex align-items-start gap-2 p-2 mb-2">
                                ...
                                <div class="d-flex align-items-center gap-1">
                                    <form
                                        action="{{ route('customer.update_cart_item', $item) }}"
                                        method="POST"
                                        class="d-inline"
                                    >
                                        @csrf
                                        ...
                                        <input
                                            ...
                                            class="quantity-input fw-semibold"
                                            @if($availableStock === 0) readonly @endif
                                        >
                                    </form>
                                    <form
                                        action="{{ route('customer.remove_cart_item', $item) }}"
                                        method="POST"
                    ...
                    <div class="mt-3 pt-3 border-top">
                        <div class="d-flex justify-content-between align-items-center mb-2">
                            ...
                            <span class="fw-bold fs-5 cart-total">Rp {{ number_format($cart->total_harga, 0, ',', '.') }}</span>
                        </div>
                    </div>
                    ...
                    <div class="d-flex flex-column flex-sm-row gap-2 mt-3">
                        ...
                        @php
                            $tenantNameForConfirm = addslashes($cart->tenant?->nama_tenant ?? 'Tenant ini');
                            $tenantNameForConfirm = str_replace("'", "\'", $tenantNameForConfirm);
                        @endphp

                        <form
                            action="{{ route('customer.checkout', $cart) }}"
                            method="POST"
                            class="flex-grow-1"
                            id="checkout-form-{{ $cart->id }}"
                        >
                            @csrf
                            <button
                                type="submit"
                                class="btn btn-success w-100"
                                onclick="return handleCheckout(this, '{{ $tenantNameForConfirm }}')"
                            >
...
function handleCheckout(button, tenantName) {
    ...
    // The form loading will be handled by our generic form loader
    return true;
}
</script>
@endpush